...
public class UserController {
	...
	public ResponseEntity<String> resetPasswordStart(@RequestBody String email) {
		...
		if (userDTO != null && userDTO.isActive()) {
			...
			Mail mail = EmailSenderService.getresetPswMail(userInformationDTO.get().getFirstName(),
					userInformationDTO.get().getLastName(), passwordToken, email);
	...
	/**
	 * When the user is trying to reach the page to reset their password with a
	 * token, check if the token exists and is valid. If it does, the user can
	 * change their password, else, the Front will put them back to the home page.
	 *
		...
		if (this.userService.findUserByPasswordResetToken(key) != null) {
			...
			if (userDTO != null) {
				if (userDTO.getTokenExpiryDate() != null
						&& this.userService.isPasswordTokenValid(userDTO.getTokenExpiryDate()) && userDTO.isActive()) {
					return ResponseEntity.ok(HttpStatus.OK);
				}
			}
		}
	 ...
	 *
	 * @param keyAndPassword the key allowing the change of the old password with
	 *                       the new one
	 * @return a ResponseEntity
	 * @author Cecile
	 */
		...
		if (keyAndPassword != null && keyAndPassword.isValid()) {
			...
			if (this.userService.isPasswordTokenValid(userDTO.getTokenExpiryDate()) && userDTO.isActive()) {
				Optional<UserInformationDTO> userInformationDTO = this.userService
						.findUserInfoByUserMail(userDTO.getMail());
				Mail mail = EmailSenderService.getNotificationMail(userInformationDTO.get().getFirstName(),
						userInformationDTO.get().getLastName(), userDTO.getMail());
